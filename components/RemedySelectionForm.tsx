...
export const RemedySelectionForm: React.FC<RemedySelectionFormProps> = ({
  ...
  // Which remedy's keynotes are we showing in the panel?
  // We track by abbreviation because that matches REMEDY_KEYNOTES.
  const [selectedRemedyAbbr, setSelectedRemedyAbbr] = useState<string | null>(null);
  ...
  // Resolve the actual remedy + keynotes to show in the panel
  const selectedRemedy =
    sortedAndFilteredRemedies.find((r) => r.abbreviation === selectedRemedyAbbr) ||
    null;

  const selectedKeynotes =
    selectedRemedy && REMEDY_KEYNOTES[selectedRemedy.abbreviation]
      ? REMEDY_KEYNOTES[selectedRemedy.abbreviation]
      : null;
      ...
      <div className="p-6 bg-slate-800/50 rounded-lg shadow-xl">
        ...
        {selectedRemedy ? (
          ...
          <p className="text-sm text-slate-300">
            Hover is disabled for now. <strong>Click</strong> on a remedy row in the
            list below to view its keynotes here.
          </p>
        )}
      </div>
            ...
            <table className="min-w-full divide-y divide-slate-700">
              ...
              <tbody className="bg-slate-800 divide-y divide-slate-700">
                {sortedAndFilteredRemedies.map((remedy) => {
                  ...
                  const isFocused =
                    selectedRemedyAbbr === remedy.abbreviation;

                  const rowClasses = [
                    'transition-all group cursor-pointer',
                    isSelected ? 'bg-cyan-900/20' : 'hover:bg-slate-700/50',
                    isFocused ? 'ring-2 ring-inset ring-cyan-500/80' : '',
                  ]
                    .filter(Boolean)
                    .join(' ');
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        setSelectedRemedyAbbr(remedy.abbreviation)
                      }
                    >
